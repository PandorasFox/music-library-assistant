...
use crate::config::{
    AlbumArtOpinions, CanonicalizationOpinions, CoverArtSanctity, CreditRoutingConfig,
    DebugOpinions, DiscExtractionOpinions, DuplicateAnalysisOpinions, ExternalMatchingConfig,
...
impl Diffable for CoverArtSanctity {
    fn diff_against(&self, other: &Self, prefix: &str, out: &mut Vec<DiffEntry>) {
        diff_leaf(self, other, prefix, out);
    }
}
...
impl_diffable_struct!(ExternalMatchingConfig {
    ...
    cover_art_sanctity, cover_art_types,
});
