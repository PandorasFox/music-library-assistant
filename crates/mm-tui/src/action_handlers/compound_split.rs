...
use crate::{compound_split_v2, helpers, progressive_worker, tag_editor, ActiveView};
...
impl App {
    ...
    pub(crate) fn start_compound_split_resolution(
        &mut self,
        safe_only: bool,
    ...
    pub(crate) fn start_compound_split_resolution_for_zone(
        &mut self,
        safe_only: bool,
        tag_filter: Option<&str>,
        zone: Zone,
        ...
        details
            ...
            .filter_map(|detail| {
                detail.mutations.iter().find_map(|m| {
                    if let mm_meta::mutations::Mutation::EmitCanonicalTag(ref ct) = m {
    ...
    fn stage_compound_canonicalize_decision(&mut self, gesture: &witness::ConfirmationGesture) {
        let (mutations, key, description) = match &self.view {
            ActiveView::CompoundTagSplit {
                ...
                let mutation = crate::compound_split_v2::types::create_canonical_signal(&state.data);
                let desc = format!(
    ...
    pub(crate) fn load_current_compound_split_signal(&mut self) -> bool {
    ...
    pub(crate) fn load_current_compound_split_signal_with_clusters(
        &mut self,
...
impl App {
    /// Start compound split resolution using the new packed query (V3).
    ...
    fn start_compound_split_resolution_v3_for_zone(
        ...
        self.view = ActiveView::CompoundTagSplitV3 {
            data,
            current_group: 0,
    ...
    fn advance_compound_split_v3(&mut self) {
        if let ActiveView::CompoundTagSplitV3 {
