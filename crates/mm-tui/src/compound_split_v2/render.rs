...
fn render_parts_pane(f: &mut Frame, area: Rect, state: &mut CompoundSplitStateV2) {
    ...
    let scroll = crate::helpers::clamp_scroll(state.part_cursor, state.part_scroll, visible_height);
    ...
    if let Some(f) = file.as_ref() {
        if let Some(ref edits_map) = state.pending_tag_edits {
            if let Some(edits) = edits_map.get(&f.inode) {
                ...
                for line in crate::helpers::render_pending_edit_lines(edits, max_width, remaining) {
