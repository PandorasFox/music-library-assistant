...
fn render_variants_pane(f: &mut Frame, area: Rect, state: &mut TagCanonicalityStateV2) {
    ...
    let scroll = crate::helpers::clamp_scroll(cursor_pos, state.variant_scroll, visible_height);
...
fn render_files_pane(f: &mut Frame, area: Rect, state: &mut TagCanonicalityStateV2) {
    ...
    let scroll = crate::helpers::clamp_scroll(state.file_cursor, state.file_scroll, visible_height);
...
fn render_tags_pane(f: &mut Frame, area: Rect, state: &TagCanonicalityStateV2) {
    ...
    let mut lines: Vec<Line> = sorted_tags
        .iter()
        .take(visible_height)
    ...
    if let Some(ref edits_map) = state.pending_tag_edits {
        if let Some(edits) = edits_map.get(&file.inode) {
            ...
            lines.extend(crate::helpers::render_pending_edit_lines(edits, max_width, remaining));
        }
    }
