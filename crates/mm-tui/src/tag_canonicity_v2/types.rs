...
pub struct TagCanonicalityStateV2 {
    ...
    pub variant_click_targets: crate::widgets::ListClickTargets,
...
impl TagCanonicalityStateV2 {
    ...
    pub fn mutations(&self) -> Vec<Mutation> {
        ...
        // Build track_info from cached file data (tag_values cached at modal init)
        let track_info: HashMap<i64, TagSet> = self
            .data
            .files
            .iter()
            .map(|f| {
                let tagset = TagSet::new(f.tag_values.iter().cloned());
                (f.inode, tagset)
            })
            .collect();
        ...
        for &inode in &self.data.inodes {
            if let Some(current_tagset) = track_info.get(&inode) {
