...
| PackReleases { incremental } | **Stage 1 orchestrator** for the release bin-packing pipeline. Loads AcoustID external matches, filters by min_confidence and duration_tolerance_pct, identifies candidate releases, loads tracklists and locale-resolved artist names, writes session manifest to `release_packing_manifest` (includes `media_count` column), truncates intermediate tables. Uses ALL recordings per inode (not just the best-confidence one), so inodes with multiple AcoustID recordings at similar confidence are linked to all possible releases. Candidates are deduplicated per (release_id, inode) keeping the highest-confidence recording. Writes `dir_file_count` (total audio files in each candidate's directory) to the candidates table. **Pinned releases**: adds pinned release IDs from `dirs.kdl` `pinned_release` fields to `all_release_ids` so their tracklists are fetched; injects synthetic candidate rows (confidence=1.0, empty recording_id) for inodes in pinned dirs that lack an AcoustID candidate for the pinned release. **Incremental mode** (`incremental=true`): after candidate dedup and pinned injection, loads MB-tagged inodes (files with both MUSICBRAINZ_TRACK and MUSICBRAINZ_RELEASE tags) and identifies "solved" releases where ALL candidate inodes are MB-tagged; solved releases (excluding pinned) are removed from the candidate set. This skips rescoring already-applied matches. **Dirty inode bridge**: before incremental filtering, checks for `"release_packing"` dirty inodes (marked by tag mutations via `PER_INODE_COMPUTATIONS`); if any exist, disables incremental skip for this run and clears the dirty flags — forces full re-scoring when tag data that affects packing score dimensions has changed. Threads `incremental` through to Stage 3a and Stage 4 for signal handling. Spawns N `ScoreReleaseCandidates` (one per release). Defers `ComputeReleaseMappings` as barrier-separated phase (which chains the remaining stages). Manual trigger only (auto-trigger after fetch uses incremental mode) |
