...
| DetectEmbeddableAlbumArt | Find directories with sidecar album art images alongside audio files lacking embedded pictures. Also detects directories where sidecar art is higher quality than existing embedded art (UpgradeableAlbumArt). **Spawned by BackfillAlbumArtInfo** (not directly by schedule) — upgrade detection depends on `pic_format`/`pic_width`/`pic_height` columns populated by backfill |
| BackfillAlbumArtInfo | Backfill picture metadata (format, resolution, count) for existing files. Single-pass dirty-inode computation seeded by v24→v25 migration. Spawns DetectEmbeddableAlbumArt after completion (with `wait_for_queue_drain` when metadata was written) |
...
| DetectEmbeddableAlbumArt | — | EmbeddableAlbumArt, UpgradeableAlbumArt | EmbeddableAlbumArt (stale, via set reconciliation), UpgradeableAlbumArt (stale, via set reconciliation). Scans directories for sidecar images, emits EmbeddableAlbumArt for artless files. Also queries files with picture metadata (`pic_format`/`pic_width`/`pic_height`) and compares against primary sidecar via `is_sidecar_better()` — emits UpgradeableAlbumArt for directories where sidecar outranks embedded art. Respects `album_art.min_acceptable_resolution` — embedded art meeting this threshold in both dimensions is excluded from upgrade signals. **Spawned by BackfillAlbumArtInfo** (not by ScheduleContentAnalysis directly) to ensure picture metadata is populated before upgrade detection queries it |
| BackfillAlbumArtInfo | DetectEmbeddableAlbumArt | — | — | Single-pass dirty-inode computation. Reads picture metadata (format, resolution, count) from audio files via lofty, writes to `audio_info` columns (`pic_format`, `pic_width`, `pic_height`, `pic_count`). Seeded by v24→v25 migration. Clears dirty flag after processing. Always spawns DetectEmbeddableAlbumArt; calls `wait_for_queue_drain` first when metadata was written |
...
| DeriveCorpusDeployStatus | — | DeployReady, DeployedHealthy, SidecarDeployReady | DeployReady, DeployedHealthy (via corpus reconciliation — scalar inode existence check, no hash). DeployedHealthy metadata includes `library_path`. Files deployed at the wrong path (stale) are skipped — DeriveDeployHealthSignals handles those via LibraryStale. Skips conflict losers: files whose computed deploy path is claimed by 2+ corpus files are excluded from DeployReady. Also skips files whose album directory has a ReleaseOverlap signal (release overlap suppression). Phase 4: discovers sidecar images using a single batch query (`get_all_corpus_images`) grouped by directory in Rust, replacing per-directory LIKE scans. Uses dirty-inode tracking (`sidecar_deploy` computation type): skips entirely when no dirty inodes exist and signals are already populated; runs incrementally (per-inode write/clear) when few directories changed; falls back to full reconciliation when dirty count is large. Image inodes are marked dirty on discovery by ScanCorpusDirectory. |
