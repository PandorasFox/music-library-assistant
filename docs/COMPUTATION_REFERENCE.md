...
| DetectEmbeddableAlbumArt | Find directories with sidecar album art images alongside audio files lacking embedded pictures. Also detects directories where sidecar art is higher quality than existing embedded art (UpgradeableAlbumArt) |
| BackfillAlbumArtInfo | Backfill picture metadata (format, resolution, count) for existing files. Single-pass dirty-inode computation seeded by v24→v25 migration |
