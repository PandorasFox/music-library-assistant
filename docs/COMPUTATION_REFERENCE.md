...
| EmitReleasePackingSignals | **Stage 3f** (after barrier). Final MIS stage. Emits per-inode `ReleasePacking` signals for ALL rounds. Records `pending_acoustid_submissions` for elimination winners. Defers `AnalyzeReleaseGaps` |
