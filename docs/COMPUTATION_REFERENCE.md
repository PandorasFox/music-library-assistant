...
| ScoreReleaseCandidates | **Stage 2** (N parallel). Per-release candidate scoring and elimination matching. Loads release tracklist, finds corpus inodes matched via AcoustID recordings linked to this release. **Candidate scoring** uses configurable `candidate_weights` (default: AcoustID confidence 0.25, duration match 0.25, tag similarity 0.15, track number match 0.10, directory cohesion 0.25). Directory cohesion = (unique candidate inodes from this dir for this release) / (total audio files in dir). Optimal per-release assignment via Hungarian algorithm (Kuhn-Munkres). After AcoustID scoring, performs per-release **elimination matching** for unfilled slots using configurable `elimination_weights` (default: duration match 0.35, tag similarity 0.20, track number match 0.30, directory cohesion 0.15; no AcoustID). Elimination uses full tag_similarity (title 0.4 + artist 0.35 + album 0.25), same as candidate scoring. Both weight sets are configurable under `release-packing` in config.kdl. Writes all candidates + optimal flags to `release_packing_scores` table with `match_method` column (`AcoustId` or `Elimination`) |
| ComputeReleaseMappings | **Stage 3a** (after barrier). Orchestrator for tiered MIS resolution. Loads optimal proposals from `release_packing_scores` and directory metadata from `release_packing_candidates`. Classifies each proposal into a quality tier: **Perfect** (all slots filled, 1:1 directory↔release mapping, no leftover files; multi-medium aware), **FullMatch** (all slots filled but cross-directory or extras), **Incomplete** (some slots filled), **Single** (single-track release). Packages state and defers `MapPerfectReleases` |
...
| MapFullMatchReleases | **Stage 3c** (after barrier). MIS on FullMatch proposals. Proposals only eligible if entire inode set unclaimed by prior rounds. Defers `MapIncompleteReleases` |
...
| MapFullMatchReleases | Defers MapIncompleteReleases | — | — | Stage 3c. MIS on FullMatch pool |
