...
| ImportGenresFromTags | Dirty-inode-aware: reads `dirty_inodes` keyed `import_genres_from_tags`, re-derives `source=FileTagImport` ledger rows for just those inodes, and clears their dirty markers. Per-inode it wipes existing FileTagImport rows for the inode before re-inserting so removed GENRE tag values don't linger. Empty dirty queue → no-op (formerly a full-corpus scan every TAGS-scope cycle). Bootstrap fallback: if the ledger has zero FileTagImport rows AND the dirty queue is empty, runs a one-shot full-corpus scan. Unresolved raws are coalesced into `unresolved_genre_observations`. Does NOT emit signals. Idempotent: upserts on `(inode, genre_id, source, kind)`. Runs in the TAGS-scope group of `ScheduleContentAnalysis`. Also queued by `EditGenreVocabularyMutation::additional_computations()` so newly-added aliases re-resolve previously-unresolved observations (note: this only re-runs dirty inodes — for a full re-resolve, requeue dirty-marking through a mutation). |
| DeriveDiscogsGenreLedger | Watermark-gated: reads `discogs_genre_ledger_watermark` from `app_metadata` and only re-processes Discogs releases whose `discogs_release_cache.fetched_at > watermark`. Walks `mb_release_discogs_links` joined with the filtered cache rows, parses each cached Discogs release JSON, fans out to inodes packed to the linked MB release(s) via `signal_release_packing`, and writes ledger rows with `source=Discogs` (`Genres` → `kind=Genre`, `Styles` → `kind=Style`). After processing, advances the watermark to `MAX(fetched_at)` so subsequent EXTERNAL-scope cycles with no new fetches are no-ops. Unresolved raws push to `unresolved_genre_observations`. Does NOT emit signals. Idempotent via the `(inode, genre_id, source, kind)` PK. Scheduled at startup (`scope = None`) or when `EXTERNAL` scope is flagged (new Discogs JSON landed). Explicitly NOT in the TAGS bucket: release-packing/tag-edit cycles that don't add new Discogs data would have been pure waste. |
...
| DeriveExternalMatches | Derive external match signals from AcoustID linkages. For each (corpus inode → recording_id) row, resolves recording metadata (title, artist credits, releases) from `mb_recording_cache` and compares against corpus tags using raw string equality. Rows whose MB recording cache hasn't landed yet are skipped (signal emitted on a later pass). Emits per-file ExternalMatch signals with classification and diffs. AcoustID is treated as linkage-only — its own metadata blobs are not consulted |
...
| ScanCorpusDirectory | VerifyTags + VerifyAudio (all indexed, if `force_check=true`) | — | — | Indexes directory entry (is_dir=1) in files table with read guard (skips write if entry already matches by zone+inode+mtime). Returns observed inodes to the Witch via Result (accumulated in tick(), consumed by queue_derivation_computations()). FileInCorpus is no longer emitted here — it's now written by the indexing executor in the same transaction as the inode insert. |
...
| DeriveCorpusSignals | — | UnindexedFile, MissingFile, HealthyFile, MovedFile | UnindexedFile, MissingFile, HealthyFile (stale); skips HealthyFile for OOB-flagged files; clears stale MovedFile for inodes that no longer qualify. **Move detection**: same-zone (disk path ≠ indexed path in `both` set); skipped for image files and for inodes with multiple paths in the zone (hardlinks). **GC backstop**: clears orphaned signals for inodes not in disk ∪ index — now includes FileInCorpus (since it's indexer-written and no longer guaranteed to be reconciled against `observed_inodes` here). Receives observed inodes from the Witch (accumulated from FS watcher). |
| UpdateCorpusFileSignals | — | FileInCorpus, UnindexedFile, MissingFile, HealthyFile | FileInCorpus, UnindexedFile, MissingFile, HealthyFile | Per-path observation: stats the path, emits FileInCorpus + reconciles other presence signals against observed disk state for that single inode. Used by mutations that touch corpus paths (e.g. ApplyTransaction post-write). The bulk `DeriveCorpusSignals` no longer touches FileInCorpus; the indexing executors do (deterministic on insert). |
...
| ImportGenresFromTags | — | — (writes ledger, not signals) | Direct table writes: `inode_genres` (source=FileTagImport — clears then re-inserts per dirty inode) + `unresolved_genre_observations` (bumped on conflict) + `dirty_inodes` (clears `import_genres_from_tags` entries per processed inode). No signal emission |
| DeriveDiscogsGenreLedger | — | — (writes ledger, not signals) | Direct table writes: `inode_genres` (source=Discogs, kind=Genre and kind=Style upserts) + `unresolved_genre_observations` (bumped on conflict) + `app_metadata` (`discogs_genre_ledger_watermark` advanced to `MAX(fetched_at)`). Reads watermark from `app_metadata`, then `discogs_release_cache`, `mb_release_discogs_links`, `signal_release_packing`, and `genre_aliases`. No signal emission |
...
| DeriveExternalMatches | — | ExternalMatch | ExternalMatch (via hash-based corpus reconciliation). For each corpus inode with AcoustID linkage rows, resolves the highest-confidence recording via `mb_recording_cache`, compares MB recording title/artist-credit/release titles against corpus tags (raw string equality), classifies as ExactMatch/ContentDiff/MetadataOnly. Inodes whose MB recording fetch hasn't completed are skipped silently and reclassified on a later pass. Triggered by EXTERNAL, TAGS, or FILES scope |
...
| DetectDeployConflicts | — | DeployConflict | DeployConflict (via hash-based aggregate reconciliation). **Audio-only**: filters `signal_healthy_file` against `get_audio_inodes_for_zone(Corpus)` so image-sidecar inodes are excluded — without tags they all collapse to `[no album artist]/cover.jpg` and would produce one giant bogus conflict that flags every cover hardlink except the alphabetical winner as Leftover. Uses inode-based signal lookup (signal.inode + metadata path). **Winner-first ordering**: within each conflict's `inodes` Vec, entry `[0]` is the alphabetical tiebreak winner by corpus path (mirroring `DeriveCorpusDeployStatus`'s winner selection), and the remaining entries are losers. `DeriveDeployHealthSignals` reads the loser set to reclassify superseded library hardlinks as Leftover. |
...
| DeriveDeployHealthSignals | — | LibraryLeftover, LibraryStale | LibraryLeftover, LibraryStale. Handles both audio and sidecar image files. Audio stale: computes expected deploy path from tags. Sidecar (image) classification: a library cover.jpg/folder.jpg is Healthy iff its parent directory appears in `library_album_dirs_with_audio` (the set of library album dirs that contain at least one audio file). Otherwise Leftover. This deliberately does NOT consult the corpus-side parent dir or pick an audio sibling — a single cover inode can be hardlinked into many unrelated library albums (CAA dedupe + hardlink deploy), so the only invariant that holds is "cover sits next to audio". Masks stale-conflicts: if a stale file's expected path is already occupied by a different inode, no stale signal is emitted (the LibraryMove would always fail). **Conflict-loser reclassification**: at entry, loads the set of loser inodes via `get_deploy_conflict_loser_inodes` (reads `DeployConflictSignal` BLOBs and skips `inodes[0]`, the winner). Any library file whose corpus inode is in this set short-circuits to Leftover regardless of tag-derived expected path, so the deploy pipeline stashes the superseded hardlink before deploying the winner. **Performance**: bulk-loads corpus tags for every library inode in one chunked IN query at the top, replacing per-file `get_tags(inode)` round trips inside `classify_audio_stale` / `compute_expected_library_path`. Per-inode fallback retained as defense in depth |
| DeriveCorpusDeployStatus | — | DeployReady, DeployedHealthy, SidecarDeployReady, SidecarDeployConflict | DeployReady, DeployedHealthy (via corpus reconciliation — scalar inode existence check, no hash). DeployedHealthy metadata includes `library_path`. Files deployed at the wrong path (stale) are skipped — DeriveDeployHealthSignals handles those via LibraryStale. Skips conflict losers: files whose computed deploy path is claimed by 2+ corpus files are excluded from DeployReady. Also skips files whose album directory has a ReleaseOverlap signal (release overlap suppression). Phase 4 (sidecars): discovers sidecar images via `get_all_corpus_images` (one batch query), groups candidates by `(library_name, album_dir, role_class)` where `role_class` collapses `cover_front` / `cover_back` to one slot per album so multi-disc covers compete instead of each emitting its own deploy. Within each slot the candidates are deduped by inode (alphabetically-first path wins per inode, eliminating phantom inode_paths aliases from broken hardlinks). One slot → DeployReady for the alphabetically-first corpus path; 2+ inodes → SidecarDeployConflict listing all competing inodes, plus DeployReady for the winner only when no candidate is already deployed and the destination is unoccupied. Sidecar phase uses its own dirty-inode gating. **Outer dirty-inode short-circuit**: at entry, reads dirty inodes for `corpus_deploy_status` (marked under TAGS\|FILES\|DEPLOY scope mutations via `witch/execution.rs` Phase 1c). If empty AND prior DeployReady/DeployedHealthy signals exist, returns immediately. Cleared via `clear_all_dirty_inodes` at the end of a successful run. **Performance**: bulk-loads tags for all corpus inodes in one chunked IN query before the per-inode iteration |
