...
| EmbeddableAlbumArt | DetectEmbeddableAlbumArt | DetectEmbeddableAlbumArt, EmbedAlbumArt | Directory with sidecar image and audio files lacking embedded art. Key: relative directory path. Metadata (bincode): `image_path`, `image_filename`, `artless_inodes[]`, `artless_paths[]`, `sidecar_images[]` (ordered: cover→front→back→alphabetical, each with path, filename, role, format, width, height) |
| UpgradeableAlbumArt | DetectEmbeddableAlbumArt | DetectEmbeddableAlbumArt, UpgradeAlbumArt | Directory with sidecar image higher quality than existing embedded art. Key: `upgrade:{relative_directory_path}`. Aggregate signal. Metadata (bincode): `sidecar` (SidecarImage), `upgradeable_inodes[]`, `upgradeable_paths[]`, `embedded_format`, `embedded_width`, `embedded_height`. Comparison via `is_sidecar_better()`: >10% pixel count difference wins outright; at similar resolution, lossless (PNG/BMP/TIFF) beats lossy (JPEG) |
