...
These rows are written by ResolveReleaseConflicts (Stage 3) for elimination-matched winners — corpus files assigned to release track slots via per-release elimination in Stage 2 rather than direct AcoustID lookup. The fingerprint + recording_id pair, once submitted, would allow future AcoustID lookups to find this match directly.
