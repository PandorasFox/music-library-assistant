...
fn config_diff_entries(old: &Config, new: &Config) -> Vec<DiffEntry> {
    ...
    macro_rules! cmp {
        ...
        (opt $label:expr, $old_val:expr, $new_val:expr) => {
            if $old_val != $new_val {
                let old_s = match $old_val {
                    Some(v) => v.to_string(),
                    None => "auto".to_string(),
                };
                let new_s = match $new_val {
                    Some(v) => v.to_string(),
                    None => "auto".to_string(),
                };
    ...
    // General
    cmp!(
        "Transcode lossy to FLAC",
        o.lossy_shit_formats_to_flac,
        n.lossy_shit_formats_to_flac
    );

    // Startup
    cmp!(
        "Force full check at startup",
        o.startup.force_check_all_files_at_startup,
        n.startup.force_check_all_files_at_startup
    );
    ...
    cmp!(
        "Single album suffix",
        &o.health_detection.single_album_suffix,
        &n.health_detection.single_album_suffix
    );
    ...
    cmp!(
        "Dup: Duration tolerance (ms)",
        o.duplicate_analysis.duration_tolerance_ms,
        n.duplicate_analysis.duration_tolerance_ms
    );
    cmp!(
        "Dup: Elide variant titles",
        o.duplicate_analysis.elide_variant_titles,
        n.duplicate_analysis.elide_variant_titles
    );
    ...
    cmp!(
        "AcoustID API key",
        &o.external_matching.acoustid_api_key,
        &n.external_matching.acoustid_api_key
    );
    cmp!(
        "AcoustID requests/sec",
        o.external_matching.requests_per_second,
        n.external_matching.requests_per_second
    );
